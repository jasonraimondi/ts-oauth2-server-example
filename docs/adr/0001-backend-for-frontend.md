...
- Lazy token refresh in the BFF interacts with the AS's RFC 9700 reuse detection: two concurrent requests would otherwise replay the same refresh token, which the AS reads as theft and answers by revoking the whole token family. The BFF **single-flights refresh per session** so concurrent requests share one rotation. This closes the race within a process; the per-process in-memory store means a multi-instance deployment still needs a shared lock to fully close it.
